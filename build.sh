...
[[ "${BUILD_ONLY}" -eq 0 ]] && release_port "${PORT}" || true
...
[ -f "sdkconfig.defaults.${TARGET}" ] && DEFAULTS_LIST="${DEFAULTS_LIST};sdkconfig.defaults.${TARGET}"
[ -f "sdkconfig.defaults.${SELECTED_BOARD}" ] && DEFAULTS_LIST="${DEFAULTS_LIST};sdkconfig.defaults.${SELECTED_BOARD}"
export SDKCONFIG_DEFAULTS="${DEFAULTS_LIST}"

idf.py -B "${BUILD_DIR}" -DFLIPPER_BOARD="${BOARD}" set-target "${TARGET}"
