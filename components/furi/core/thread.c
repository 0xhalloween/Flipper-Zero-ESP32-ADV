...
__attribute__((__noreturn__)) void furi_thread_catch(void) {
    __asm__ volatile("nop"); // extra magic
...
#endif
        furi_thread_diag_low_stack(thread, stack_watermark);
    }
...
bool furi_thread_enumerate(FuriThreadList* thread_list) {
    ...
    do {
        ...
        TaskStatus_t* task = malloc(count * sizeof(TaskStatus_t));
        ...
        for(uint32_t i = 0U; i < count; i++) {
            // ESP32: No direct TCB access, use simplified version
            ...
            item->stack_address = 0; // ESP32: TCB stack address not easily accessible
            size_t thread_heap = memmgr_heap_get_thread_memory(thread_id);
            item->heap = thread_heap == MEMMGR_HEAP_UNKNOWN ? 0u : thread_heap;
            item->stack_size = 0; // ESP32: stack size not easily accessible from TCB
...
void furi_thread_stdin_unread(char* buffer, size_t size) {
    ...
    FuriString* new_buf = furi_string_alloc();
