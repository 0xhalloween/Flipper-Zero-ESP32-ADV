/**
 * @file furi_hal_sd.c
 * SD Card HAL — shared implementation for all supported boards.
 *
 * Uses ESP-IDF SDSPI driver + raw FatFs diskio registration.
 * Supports both shared-bus (SPI2) and dedicated-bus (SPI3) SD configurations
 * via BOARD_SD_SPI_HOST defined in the active board header.
 */
...
/* Allow boards to specify a dedicated SPI host for the SD card.
 * - T-Embed / Waveshare: SD shares SPI2_HOST with the LCD (legacy path).
 * - M5Stack Cardputer: SD is on SPI3_HOST; board header sets BOARD_SD_SPI_HOST. */
#if defined(BOARD_SD_SPI_HOST)
#  define SD_SPI_HOST    BOARD_SD_SPI_HOST
#  define SD_SEPARATE_BUS 1  /* SD has its own bus — initialise it below */
#else
#  define SD_SPI_HOST    SPI2_HOST
#  define SD_SEPARATE_BUS 0
#endif
...
static bool sd_prepare_card(void) {
...
#if !SD_SEPARATE_BUS
    /* Shared-bus boards: skip SD if the SPI2 bus is reserved for an external
     * CC1101 or similar peripheral that owns the same pins. */
    ...
    ESP_LOGI(TAG, "Initializing SD card on SPI%d_HOST, CS=GPIO%d",
             (SD_SPI_HOST == SPI2_HOST) ? 2 : 3, gpio_sdcard_cs.pin);

#if SD_SEPARATE_BUS
    /* For boards where SD is on a dedicated SPI host (not shared with LCD),
     * we must initialise that bus ourselves.  The LCD init only touches
     * BOARD_LCD_SPI_HOST (SPI2), so SPI3 is uninitialised at this point.
     *
     * IMPORTANT: Drive CS HIGH before touching the bus.  If CS floats low
     ...
     * and its state machine gets stuck — it will never reply to CMD0. */
    {
        ...
        gpio_set_level((gpio_num_t)BOARD_PIN_SD_CS, 1);   /* CS = HIGH = deasserted */
        vTaskDelay(pdMS_TO_TICKS(1));                      /* settle */

        spi_bus_config_t sd_bus_cfg = {
            .mosi_io_num   = BOARD_PIN_SD_MOSI,
            .miso_io_num   = BOARD_PIN_SD_MISO,
            .sclk_io_num   = BOARD_PIN_SD_SCLK,
            .quadwp_io_num = -1,
            .quadhd_io_num = -1,
            .max_transfer_sz = 4000,
        };
        
        gpio_set_pull_mode((gpio_num_t)BOARD_PIN_SD_CS, GPIO_PULLUP_ONLY);
        gpio_set_pull_mode((gpio_num_t)BOARD_PIN_SD_MISO, GPIO_PULLUP_ONLY);
        gpio_set_pull_mode((gpio_num_t)BOARD_PIN_SD_MOSI, GPIO_PULLUP_ONLY);
        gpio_set_pull_mode((gpio_num_t)BOARD_PIN_SD_SCLK, GPIO_PULLUP_ONLY);
        
        esp_err_t bus_err = spi_bus_initialize(SD_SPI_HOST, &sd_bus_cfg, SDSPI_DEFAULT_DMA);
        if(bus_err == ESP_ERR_INVALID_STATE) {
            ESP_LOGI(TAG, "SD SPI bus already initialized, sharing host=%d", SD_SPI_HOST);
    ...
    /* Use SD_MAX_FREQ (20 MHz) instead of probing freq so the driver can negotiate higher speed */
    host.max_freq_khz = SD_MAX_FREQ;

    ESP_LOGI(
        TAG,
        "SDSPI config: host=%d cs=%d target_probe_freq_khz=%d",
        dev_cfg.host_id,
        dev_cfg.gpio_cs,
        host.max_freq_khz);

    /* SD power-on settling: the SD card VDD must be stable for ≥1ms before
     * CMD0, and the SPI clock must present ≥74 idle clocks.  The sdspi
     * driver sends the 74 clocks, but does not add a VDD-settle delay.
     * 10ms covers even slow-rising power rails on battery-backed designs. */
    vTaskDelay(pdMS_TO_TICKS(10));

    esp_err_t ret = ESP_OK;

    /* NOTE: Do NOT take furi_hal_spi_bus_lock() here.  That mutex guards the
     * shared SPI2 (LCD) bus.  The SD card is on the independent SPI3 bus and
     * has its own device handle — locking SPI2 adds zero safety and risks a
     * priority-inversion stall if the display task already holds it. */
    do {
        ret = sdspi_host_init();
        if(ret != ESP_OK) {
        ...
        if(ret != ESP_OK) {
            ESP_LOGE(TAG, "sdmmc_card_init failed: %s — check card insertion and CS wiring", esp_err_to_name(ret));
        }
    } while(false);
...
FuriStatus furi_hal_sd_info(FuriHalSdInfo* info) {
    ...
    info->product_revision_major = (sd_card->cid.oem_id >> 4) & 0x0F;
    info->product_revision_minor = sd_card->cid.oem_id & 0x0F;
