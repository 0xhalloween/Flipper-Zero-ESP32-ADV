...
#if defined(BOARD_LCD_SWAP_XY) && BOARD_LCD_SWAP_XY
/* For landscape screens like Cardputer:
 * Flipper 128x64 scaled to 240x120 (1.875x) */
#define SCALED_WIDTH  240
#define SCALED_HEIGHT 120
#else
/* For portrait screens:
 * Flipper 64x128 scaled to 120x240 */
...
/* Centering margins */
#if defined(BOARD_LCD_SWAP_XY) && BOARD_LCD_SWAP_XY
/* 128x64 scaled by 1.875x -> 240x120. Fits Cardputer 240x135 with margins. */
#undef SCALED_WIDTH
#undef SCALED_HEIGHT
#define SCALED_WIDTH  240
#define SCALED_HEIGHT 120
#define MARGIN_X      0
#define MARGIN_Y      ((BOARD_LCD_V_RES - SCALED_HEIGHT) / 2)
#else
...
void furi_hal_display_init(void) {
    ...
    spi_bus_config_t bus_cfg = {
        ...
        /* Some boards (e.g. Cardputer) use a separate SPI host for SD and
         * don't need MISO on the LCD bus.  Such boards define
         * BOARD_PIN_LCD_SPI_MISO as -1.  Boards that share the SPI bus
         * between LCD and SD fall back to the SD MISO pin (original path). */
#if defined(BOARD_PIN_LCD_SPI_MISO)
        .miso_io_num = BOARD_PIN_LCD_SPI_MISO,
#else
