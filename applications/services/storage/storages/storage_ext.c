...
static void make_sdcard_path(char* out, size_t out_size, const char* path) {
    if (path[0] == '/') {
        snprintf(out, out_size, "0:/sdcard%s", path);
    } else if (path[0] == '\0') {
        snprintf(out, out_size, "0:/sdcard");
    } else {
        snprintf(out, out_size, "0:/sdcard/%s", path);
    }
    
    // FatFs rejects trailing slashes on non-root directories
    size_t len = strlen(out);
    if (len > 9 && out[len - 1] == '/') { // 9 is length of "0:/sdcard"
        out[len - 1] = '\0';
    }
}
...
static bool sd_remove_recursive(const char* path) {
    ...
    char sd_path[256];
    make_sdcard_path(sd_path, sizeof(sd_path), path);
    FuriString* current_path = furi_string_alloc_set(sd_path);
    ...
    while(true) {
        ...
        if(!furi_string_equal_str(current_path, sd_path)) {
...
static bool storage_ext_file_open(
    ...
    file->internal_error_id = f_open(file_data, sd_path, _mode);
...
static bool storage_ext_dir_open(void* ctx, File* file, const char* path) {
    ...
    file->internal_error_id = f_opendir(file_data, sd_path);
    file->error_id = storage_ext_parse_error(file->internal_error_id);
    return file->error_id == FSE_OK;
}
...
static FS_Error storage_ext_common_stat(void* ctx, const char* path, FileInfo* fileinfo) {
    ...
    char sd_path[256];
    make_sdcard_path(sd_path, sizeof(sd_path), path);
    SDError result = f_stat(sd_path, &_fileinfo);
...
static FS_Error storage_ext_common_remove(void* ctx, const char* path) {
    ...
    char sd_path[256];
    make_sdcard_path(sd_path, sizeof(sd_path), path);
    SDError result = f_unlink(sd_path);
    return storage_ext_parse_error(result);
#endif
}
...
static FS_Error storage_ext_common_mkdir(void* ctx, const char* path) {
    ...
    char sd_path[256];
    make_sdcard_path(sd_path, sizeof(sd_path), path);
    SDError result = f_mkdir(sd_path);
    return storage_ext_parse_error(result);
#endif
}
